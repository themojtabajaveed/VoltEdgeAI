...
def _build_section_0_health(
    ...
    # FIX-1: File-based pre-market brief check
    if today is None:
        import zoneinfo
    ...
    # FIX-3: VIPER scan health from signals file
    ...
    if os.path.exists(signals_path):
        try:
            ...
            viper_health_str = "signals file unreadable"
    else:
...
def _build_section_4_trades(db_ctx: dict) -> str:
    ...
    if not trades:
        ...
        lines.append(
            ...
            f"- Conviction threshold ({_threshold:.0f}) was never reached\n"
...
def generate_post_market_report(
    ...
    # FIX-2: Compute phase from live snapshot, same as mid_session_pulse
    phase_str = "UNKNOWN"
    try:
    ...
    conviction_data = {"signals": [], "phase": phase_str}
    if conviction_engine:
        try:
            all_signals = []
    ...
    try:
        ...
        try:
            ...
            pass
        mkt_summary = f"Phase: {phase_str} | Nifty: N/A | VIX: N/A"
